...
# from vnet.common.register.srv_register import build_idle_busy_registrar_from_env
...
class PromptServer():
    def __init__(self, loop):
        ...
        self._idle_busy_registrar = None

        import vnet.server_common_srv as server_common_srv
        server_common_srv.define_view(routes,self)
        ...
        async def post_prompt(request):
            ...
            if "prompt" in json_data:
                ...
                if valid[0]:
                    ...
                    # queue changed, notify (triggers idle/busy registrar)
                    self.queue_updated()
    ...
    def queue_updated(self):
        ...
        # Update Nacos idle/busy registration if registrar is available
        try:
            if self._idle_busy_registrar is not None:
                remaining = self.prompt_queue.get_tasks_remaining()
                self._idle_busy_registrar.update(is_idle=(remaining == 0))
        except Exception:
            pass
    ...
    async def start_multi_address(self, addresses, call_on_start=None, verbose=True):
        ...
        # Build registrar after address/port known, then update state
        try:
            # self._idle_busy_registrar = build_idle_busy_registrar_from_env(getattr(self, 'address', '127.0.0.1'), int(getattr(self, 'port', 0)))
            self.queue_updated()
        except Exception:
            pass
